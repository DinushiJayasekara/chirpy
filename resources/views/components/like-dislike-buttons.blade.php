<div class="flex">
    <form action="/chirps/{{ $chirp->id }}/like" method="post">
        @csrf

        <button type="submit" class="text-xs">
            <div
                class="flex items-center mr-3 {{ $chirp->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    <form action="/chirps/{{ $chirp->id }}/like" method="post">
        ...
        <button type="submit" class="text-xs">
            <div
                class="flex items-center {{ $chirp->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
                <svg viewBox="0 0 20 20" class="mr-1 w-4">
                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                        <g class="fill-current">
                            <path
